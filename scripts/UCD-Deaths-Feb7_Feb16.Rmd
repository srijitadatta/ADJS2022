...
deaths %>% 
  ...
  arrange(desc(total_deaths))
```
Which state had the highest death rate? 
```{r}
deaths %>%
  filter(year==2020) %>%
...
deaths %>% filter(grepl("firearm", icd_10_113_cause_list, ignore.case=TRUE)) %>% count(icd_10_113_cause_list)
...
deaths %>% 
  ...
  filter(grepl("substance|drug|firearm", icd_10_113_cause_list, ignore.case=TRUE))
```
...
Which causes of death are increasing from year to year? 
```{r}
chg_in_cause <- deaths %>%
...
deaths %>% 
  group_by(state, year) %>%
  summarise(totals_deaths = sum(num_deaths, na.rm=T)) %>%
